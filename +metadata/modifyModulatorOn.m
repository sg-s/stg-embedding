...
modulators = intersect(sourcedata.modulators,fieldnames(data));

if length(modulators)>1

	first_index = NaN(length(modulators),1);
	for i = 1:length(modulators) 
		fidx = find(data.(modulators{i}) > 0, 1, 'first');
		if ~isempty(fidx)
			first_index(i) = fidx;
		end
	end
	[~,use_me] = nanmin(first_index);
	modulators = modulators(use_me);
end
assert(length(modulators)==1,'Wrong # of modulators')
modulator = modulators{1};
